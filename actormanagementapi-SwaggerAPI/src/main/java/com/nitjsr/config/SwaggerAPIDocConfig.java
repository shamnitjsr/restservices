...
public class SwaggerAPIDocConfig {
	...
	public Docket createDocket() {
		return new Docket(DocumentationType.SWAGGER_2) // Swagger UI type
				.select().apis(RequestHandlerSelectors.basePackage("com.nt.rest")) // to specify base pkg of
																					// @RestController clases
				.paths(PathSelectors.regex("/actor.*")) // global path
				.build() // Buids Docket obj
				.useDefaultResponseMessages(true).apiInfo(createAPIInfo());
	}
	...
	private ApiInfo createAPIInfo() {
		...
		return new ApiInfo("Actor-API Doc1", "API Info Actor API", "5.7.RELEASE", "http://www.nareshit.com/license",
